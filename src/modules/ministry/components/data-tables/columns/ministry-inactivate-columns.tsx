...
import { MinistryInactivateCard } from '@/modules/ministry/components/cards/inactivate/MinistryInactivateCard';
...
export const ministryInactivateColumns: Array<ColumnDef<MinistryColumns, any>> = [
...

  {
    ...
    cell: (info) => {
      const id = info.row.original.id;
      return info.getValue() === '-' ? '-' : <MinistryInactivateCard idRow={id} />;
    },
    header: () => {
      return (
        <Button
