...
type TitleColor = 'slate' | 'green' | 'blue' | 'sky' | 'orange' | 'red' | 'amber';

interface MinistryModuleHeaderProps {
  title: string;
  description: string;
...
export const MinistryModuleHeader = ({
  title,
  description,
  titleColor = 'slate',
  className,
}: MinistryModuleHeaderProps): JSX.Element => {
  return (
    <div
      className={cn(
      ...
      <div className='relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        ...
        <div className='hidden md:flex items-center justify-center'>
          <div className='p-4 bg-slate-200/70 dark:bg-white/10 rounded-2xl backdrop-blur-sm'>
            <MdGroups3 className='w-12 h-12 text-slate-600 dark:text-white/90' />
          </div>
        </div>
      </div>
    </div>
