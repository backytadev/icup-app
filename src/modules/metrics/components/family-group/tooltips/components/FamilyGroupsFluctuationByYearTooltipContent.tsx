...
export const FamilyGroupsFluctuationByYearTooltipContent = (
  ...
  return (
    <div className='min-w-[190px] max-w-[250px] rounded-xl border border-slate-200/80 dark:border-slate-700/60 bg-white dark:bg-slate-900 overflow-hidden shadow-lg'>
      {/* Header */}
      <div className='px-3 py-2 bg-gradient-to-r from-green-50 to-green-50/30 dark:from-green-900/20 dark:to-green-900/10 border-b border-slate-200/60 dark:border-slate-700/40'>
        <div className='flex items-center gap-2'>
          <div className='p-1 rounded-md bg-green-500/10 dark:bg-green-500/20'>
            <TbChartBar className='w-3.5 h-3.5 text-green-600 dark:text-green-400' />
          </div>
            ...
            <div key={`item-${index}`} className='flex items-center justify-between gap-3'>
              <div className='flex items-center gap-2'>
                ...
                <span className='font-inter text-[11px] font-medium text-slate-600 dark:text-slate-300'>
                  {entry.name === 'newFamilyGroups' ? 'Nuevos' : 'Bajas'}
                </span>
              </div>
