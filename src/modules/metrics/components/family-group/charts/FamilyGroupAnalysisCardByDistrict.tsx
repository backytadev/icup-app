...
export const FamilyGroupAnalysisCardByDistrict = (): JSX.Element => {
  //* Context
  ...
  const familyGroupsByDistrictQuery = useQuery({
    queryKey: ['family-groups-by-district', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  useEffect(() => {
    if (familyGroupsByDistrictQuery?.data) {
      const districts = Object.values(familyGroupsByDistrictQuery?.data).map(
        (payload) => payload.district
      );

      setCurrentDistricts([...new Set(districts)]);
    }
  }, [familyGroupsByDistrictQuery?.data, activeChurchId]);
  ...
  const isFetchingData =
    !searchParams || (familyGroupsByDistrictQuery?.isFetching && !mappedData?.length);
