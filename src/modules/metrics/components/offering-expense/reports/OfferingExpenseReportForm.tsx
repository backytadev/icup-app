...
import {
  ...
  MetricOfferingExpenseSearchTypeNames,
} from '@/modules/metrics/enums/metrics-search-type.enum';
import { offeringExpenseReportFormSchema } from '@/modules/metrics/schemas/report-form-schema';
import { getOfferingExpenseMetricsReport } from '@/modules/metrics/services/offering-expense-metrics.service';
...
export const OfferingExpenseReportForm = ({ dialogClose }: Props): JSX.Element => {
  ...
  //* Form
  const form = useForm<z.infer<typeof offeringExpenseReportFormSchema>>({
    mode: 'onChange',
    resolver: zodResolver(offeringExpenseReportFormSchema),
    defaultValues: {
      types: [MetricOfferingExpenseSearchType.OperationalOfferingExpenses],
  ...
  const generateReportQuery = useQuery({
    queryKey: ['offering-expenses-metrics-report', church],
    queryFn: () =>
      getOfferingExpenseMetricsReport({
        churchId: church ?? '',
        year: year ?? '',
        ...
        <div className='flex items-center justify-between gap-3'>
          <div className='flex-1 min-w-0'>
            ...
            <p className='text-amber-100/80 text-[12px] md:text-[13px] font-inter mt-0.5'>
              Selecciona las opciones para el reporte PDF de gastos de ofrendas.
            </p>
          </div>
                ...
                render={() => (
                  <FormItem>
                    ...
                    <div className='flex flex-col md:grid md:grid-cols-2 items-start md:items-center mx-auto gap-x-[5rem] justify-between gap-y-2'>
                      {Object.values(MetricOfferingExpenseSearchType).map((type) => (
                        <FormField
                          key={type}
                          control={form.control}
                          name='types'
                            ...
                            return (
                              ...
                              >
                                <FormControl className='text-[14px] md:text-[14px]'>
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      let updatedTypes: MetricOfferingExpenseSearchType[] = [];
                                      checked
                                ...
                                <FormLabel className='text-[13px] md:text-[14px] font-medium font-inter cursor-pointer text-slate-600 dark:text-slate-400'>
                                  {MetricOfferingExpenseSearchTypeNames[type]}
                                </FormLabel>
                              </FormItem>
                            );
                          }}
