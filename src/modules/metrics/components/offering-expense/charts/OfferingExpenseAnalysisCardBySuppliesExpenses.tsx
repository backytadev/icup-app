...
import { getSuppliesOfferingExpenses } from '@/modules/metrics/services/offering-expense-metrics.service';
...
export const OfferingExpenseAnalysisCardBySuppliesExpenses = (): JSX.Element => {
  //* Context
  ...
  const suppliesOfferingExpenses = useQuery({
    queryKey: ['supplies-offering-expenses', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData = !searchParams || (suppliesOfferingExpenses?.isFetching && !mappedData?.length);
  ...
  return (
    <MetricCard
      ...
      description='Gastos en suministros del mes.'
      icon={<TbBox className='w-5 h-5 text-teal-600 dark:text-teal-400' />}
