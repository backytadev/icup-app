...
export const OfferingExpenseAnalysisCardByMaintenanceAndRepairExpenses = (): JSX.Element => {
  //* Context
  ...
  const maintenanceAndRepairOfferingExpenses = useQuery({
    queryKey: ['maintenance-and-repair-offering-expenses', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData = !searchParams || (maintenanceAndRepairOfferingExpenses?.isFetching && !mappedData?.length);
  ...
  return (
    <MetricCard
      ...
      description='Gastos de mantenimiento y reparación.'
      icon={<TbTool className='w-5 h-5 text-orange-600 dark:text-orange-400' />}
