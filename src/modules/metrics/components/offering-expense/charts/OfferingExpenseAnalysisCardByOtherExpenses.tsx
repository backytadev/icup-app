...
export const OfferingExpenseAnalysisCardByOthersExpenses = (): JSX.Element => {
  //* Context
  ...
  const othersOfferingExpenses = useQuery({
    queryKey: ['offering-others-expenses', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData = !searchParams || (othersOfferingExpenses?.isFetching && !mappedData?.length);
  ...
  return (
    <MetricCard
      ...
      description='Otros gastos registrados del mes.'
      icon={<TbList className='w-5 h-5 text-lime-600 dark:text-lime-400' />}
