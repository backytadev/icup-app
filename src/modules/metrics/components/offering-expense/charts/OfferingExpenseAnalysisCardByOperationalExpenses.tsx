...
export const OfferingExpenseAnalysisCardByOperationalExpenses = (): JSX.Element => {
  //* Context
  ...
  const operationalOfferingExpenses = useQuery({
    queryKey: ['operational-offering-expenses', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData = !searchParams || (operationalOfferingExpenses?.isFetching && !mappedData?.length);
  ...
  return (
    <MetricCard
      ...
      description='Gastos operativos de la iglesia del mes.'
      icon={<TbSettings className='w-5 h-5 text-green-600 dark:text-green-400' />}
