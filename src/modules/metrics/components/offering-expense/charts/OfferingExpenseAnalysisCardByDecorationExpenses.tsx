...
export const OfferingExpenseAnalysisCardByDecorationExpenses = (): JSX.Element => {
  //* Context
  ...
  const decorationOfferingExpenses = useQuery({
    queryKey: ['decoration-offering-expenses', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData = !searchParams || (decorationOfferingExpenses?.isFetching && !mappedData?.length);
  ...
  return (
    <MetricCard
      ...
      description='Gastos en artículos decorativos del mes.'
      icon={<TbPalette className='w-5 h-5 text-purple-600 dark:text-purple-400' />}
