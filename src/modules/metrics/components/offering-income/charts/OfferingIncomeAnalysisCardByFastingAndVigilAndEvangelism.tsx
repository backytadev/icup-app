...
export const OfferingIncomeAnalysisCardByFastingAndVigilAndEvangelism = (): JSX.Element => {
  //* Context
  ...
  const offeringIncomeByFastingAndVigil = useQuery({
    queryKey: ['offering-income-by-fasting-and-vigil', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData =
    !searchParams || (offeringIncomeByFastingAndVigil?.isFetching && !offeringIncomeByFastingAndVigil?.data?.length);
  const isEmptyData = !isFetchingData && !offeringIncomeByFastingAndVigil?.data?.length;
  ...
  return (
    <MetricCard
      ...
      title={
        <>
          <span>A.V.E</span>
          <span className='text-base sm:text-lg font-normal text-slate-600 dark:text-slate-300'>
            (Ayunos, Vigilias, Evangelismos)
          </span>
      ...
      description='Ofrendas por tipo de actividad espiritual.'
      icon={<TbFlame className='w-5 h-5 text-yellow-500 dark:text-yellow-400' />}
