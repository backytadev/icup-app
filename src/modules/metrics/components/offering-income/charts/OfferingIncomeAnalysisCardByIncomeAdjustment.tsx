...
import { getOfferingIncomeAdjustment } from '@/modules/metrics/services/offering-income-metrics.service';
import { OfferingIncomeAdjustmentTooltipContent } from '@/modules/metrics/components/offering-income/tooltips/components/OfferingIncomeAdjustmentTooltipContent';
...
export const OfferingIncomeAnalysisCardByIncomeAdjustment = (): JSX.Element => {
  ...
  //* Queries
  const offeringIncomeAdjustment = useQuery({
    queryKey: ['offering-income-adjustment', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  }, [offeringIncomeAdjustment?.data, year]);
  ...
  const isFetchingData =
    !searchParams || (offeringIncomeAdjustment?.isFetching && !offeringIncomeAdjustment?.data?.length);
  const isEmptyData = !isFetchingData && !offeringIncomeAdjustment?.data?.length;
  ...
  return (
    <MetricCard
      ...
      description='Registros de ajuste de ingresos.'
      icon={<TbRefresh className='w-5 h-5 text-teal-600 dark:text-teal-400' />}
    ...
    >
      ...
      >
        <BarChart
          accessibilityLayer
          data={offeringIncomeAdjustment?.data}
        ...
        >
          ...
          <ChartTooltip
            ...
            content={OfferingIncomeAdjustmentTooltipContent as any}
          />
          <ChartLegend
            content={
