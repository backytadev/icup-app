...
export const OfferingIncomeAnalysisCardByYouthService = (): JSX.Element => {
  //* Context
  ...
  const offeringIncomeByYouthService = useQuery({
    queryKey: ['offering-income-by-youth-service', { ...searchParams, church: activeChurchId }],
    queryFn: () => {
  ...
  const isFetchingData =
    !searchParams || (offeringIncomeByYouthService?.isFetching && !offeringIncomeByYouthService?.data?.length);
  const isEmptyData = !isFetchingData && !offeringIncomeByYouthService?.data?.length;
  ...
  return (
    <MetricCard
      ...
      description='Ofrendas del culto de jóvenes del mes.'
      icon={<TbBolt className='w-5 h-5 text-violet-600 dark:text-violet-400' />}
