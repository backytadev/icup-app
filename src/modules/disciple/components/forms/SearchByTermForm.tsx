...
import { discipleSearchByTermFormSchema } from '@/modules/disciple/schemas';
import { type DiscipleSearchFormByTerm } from '@/modules/disciple/types';
...
interface SearchByTermFormProps {
  onSearch: (params: DiscipleSearchFormByTerm, formData: DiscipleSearchFormByTerm) => void;
}
...
export const SearchByTermForm = ({ onSearch }: SearchByTermFormProps): JSX.Element => {
  ...
  const handleSubmit = (formData: z.infer<typeof discipleSearchByTermFormSchema>): void => {
    let newDateTermTo;
    ...
    onSearch(searchParams as DiscipleSearchFormByTerm, formData as DiscipleSearchFormByTerm);
  ...
  const getInputLabel = (): string => {
    switch (searchType) {
      case DiscipleSearchType.OriginCountry:
        return 'País (origen)';
      case DiscipleSearchType.ZoneName:
        return 'Nombre de Zona';
      case DiscipleSearchType.FamilyGroupCode:
        return 'Código de Grupo Familiar';
      case DiscipleSearchType.FamilyGroupName:
        return 'Nombre de Grupo Familiar';
      case DiscipleSearchType.ResidenceCountry:
        return 'País (residencia)';
      case DiscipleSearchType.ResidenceDepartment:
        return 'Departamento (residencia)';
      case DiscipleSearchType.ResidenceProvince:
        return 'Provincia (residencia)';
      case DiscipleSearchType.ResidenceDistrict:
        return 'Distrito (residencia)';
      case DiscipleSearchType.ResidenceUrbanSector:
        return 'Sector Urbano (residencia)';
      case DiscipleSearchType.ResidenceAddress:
  ...
  const showFirstNamesField =
    searchType === DiscipleSearchType.FirstNames || searchType === DiscipleSearchType.FullNames;

  const showLastNamesField =
    searchType === DiscipleSearchType.LastNames || searchType === DiscipleSearchType.FullNames;
  ...
  const getSelectLabel = (): string => {
    switch (searchType) {
      case DiscipleSearchType.Gender:
        return 'Género';
      case DiscipleSearchType.BirthMonth:
        return 'Mes de nacimiento';
      case DiscipleSearchType.MaritalStatus:
        return 'Estado civil';
      default:
  ...
  const getSelectOptions = (): Record<string, string> => {
    switch (searchType) {
      case DiscipleSearchType.Gender:
        return DiscipleSearchNamesByGender;
      case DiscipleSearchType.BirthMonth:
        return DiscipleSearchNamesByBirthMonth;
      case DiscipleSearchType.MaritalStatus:
        return DiscipleSearchNamesByMaritalStatus;
      default:
        return DiscipleSearchNamesByRecordStatus;
    }
  };
