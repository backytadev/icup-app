...
import { supervisorFormSchema } from '@/modules/supervisor/schemas';
import {
  supervisorCreateDefaultValues,
  supervisorUpdateDefaultValues,
} from '@/modules/supervisor/constants';
...
} from '@/modules/supervisor/hooks/mutations';
...
type UseSupervisorFormOptions = CreateModeOptions | UpdateModeOptions;

export interface UseSupervisorFormReturn {
  form: UseFormReturn<SupervisorFormData>;
  ...
  //* Alert dialog state — copastor relation change (update mode)
  ...
  //* Handlers
  handleSubmit: (formData: SupervisorFormData) => void;
  handleRolePromotion: () => void;
}

export const useSupervisorForm = (options: UseSupervisorFormOptions): UseSupervisorFormReturn => {
  const { mode } = options;
  ...
  //* Alert dialog state — triggers when user picks a different copastor
  const [changedId, setChangedId] = useState<string | undefined>(
    mode === 'update' ? (options as UpdateModeOptions).data?.theirCopastor?.id : undefined
  );
  ...
  const form = useForm<SupervisorFormData>({
    ...
    defaultValues:
      mode === 'create' ? supervisorCreateDefaultValues : supervisorUpdateDefaultValues,
  });
  ...
  //* Ensure Supervisor role is always set on create
  useEffect(() => {
    if (mode === 'create') {
      form.setValue('roles', [MemberRole.Supervisor]);
    }
  }, [form, mode]);
    ...
    form.reset({
      ...
      theirCopastor:
        ...
        relTypeValue === RelationType.RelatedBothMinistriesAndHierarchicalCover
          ? (updateData.theirCopastor?.id ?? '')
          : '',
      theirPastorRelationDirect:
        relTypeValue === RelationType.RelatedDirectToPastor
  ...
  useEffect(() => {
    ...
    url.pathname = `/supervisors/update/${updateId}/edit`;
  ...
  //* Open copastor-change alert dialog when user picks a different copastor
  useEffect(() => {
    if (mode !== 'update') return;

    if (updateData?.theirCopastor?.id !== changedId && changedId !== undefined) {
      setTimeout(() => {
  ...
  //* Clear relation fields whenever relation type changes (user must re-select)
  useEffect(() => {
    if (
  ...
  useEffect(() => {
    ...
    //* RelatedDirectToPastor: needs theirPastorRelationDirect
    if (relationType === RelationType.RelatedDirectToPastor) {
    ...
    //* OnlyRelatedHierarchicalCover: needs theirCopastor
    ...
    //* OnlyRelatedMinistries: needs theirPastorOnlyMinistries + all ministry blocks complete
    if (relationType === RelationType.OnlyRelatedMinistries) {
    ...
    //* RelatedBothMinistriesAndHierarchicalCover: needs theirCopastor + all ministry blocks complete
    if (relationType === RelationType.RelatedBothMinistriesAndHierarchicalCover) {
      const allBlocksComplete = ministryBlocks.every(
        (b) => b.churchId && b.ministryId && b.ministryType && b.ministryRoles.length > 0
      );

      if (!theirCopastor || !allBlocksComplete) {
        setIsSubmitButtonDisabled(true);
        setIsMessageErrorDisabled(true);
      } else {
        setIsSubmitButtonDisabled(false);
  ...
  useEffect(() => {
    ...
    //* After promotion to Copastor: needs theirPastor
    ...
    //* Supervisor still — RelatedDirectToPastor
    if (
    ...
    //* Supervisor still — OnlyRelatedHierarchicalCover
    if (
      roles.includes(MemberRole.Supervisor) &&
      relationType === RelationType.OnlyRelatedHierarchicalCover
    ) {
    ...
    //* Supervisor still — OnlyRelatedMinistries
    if (
      roles.includes(MemberRole.Supervisor) &&
    ...
    //* Supervisor still — RelatedBothMinistriesAndHierarchicalCover
    if (
      roles.includes(MemberRole.Supervisor) &&
      relationType === RelationType.RelatedBothMinistriesAndHierarchicalCover
    ) {
  ...
  //* Enabled only when:
  //*   - update mode
  //*   - form fields match server data (isDirty = false)
  //*   - ministry blocks have not changed from server data
  //*   - record is active
  //*   - no mutation in progress (isInputDisabled = false)
  ...
  const handleSubmit = (formData: SupervisorFormData): void => {
    ...
    if (mode === 'create') {
      ...
      supervisorCreationMutation.mutate({ ...formData, theirMinistries: resolvedMinistries });
    } else {
      setIsInputDisabled(true);
      setIsSubmitButtonDisabled(true);
  ...
  //* Inline role-promotion handler.
  //* Switches the supervisor's role to Copastor and locks the form for confirmation.
  ...
  const handleRolePromotion = (): void => {
    ...
    const isSupervisor = currentRoles.includes(MemberRole.Supervisor);
    ...
    if (isSupervisor && isEligibleForCopastor) {
      const updatedRoles = currentRoles.filter(
        (r) => r !== MemberRole.Supervisor && r !== MemberRole.Treasurer
      );
      form.setValue('roles', [...updatedRoles, MemberRole.Copastor]);
    }
