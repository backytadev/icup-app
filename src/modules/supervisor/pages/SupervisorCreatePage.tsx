...
export const SupervisorCreatePage = (): JSX.Element => {
  ...
  const handleSubmit = (formData: z.infer<typeof supervisorFormSchema>): void => {
    ...
    supervisorCreationMutation.mutate({
      ...formData,
                  ...
                  return (
                    <FormItem className='mt-3'>
                      ...
                      >
                        ...
                        <SelectContent>
                          {Object.entries(RelationTypeModuleNames['supervisor']).map(
                            ([key, value]) => (
                              <SelectItem className={`text-[14px]`} key={key} value={key}>
                                {value}
                              </SelectItem>
                          ...
                          <PopoverTrigger asChild>
                            <FormControl className='text-[14px] md:text-[14px]'>
                              ...
                              >
                                {field.value
                                  ? `${
                                      pastorsQuery.data?.find((pastor) => pastor.id === field.value)
                                        ?.member?.firstNames
                                    } ${pastorsQuery.data?.find((pastor) => pastor.id === field.value)?.member?.lastNames}`
                                  ...
                                  <CommandGroup className='max-h-[200px] h-auto'>
                                    {pastorsQuery.data?.map((pastor) => (
                                      <CommandItem
                                        ...
                                        key={pastor.id}
                                        onSelect={() => {
                                          form.setValue('theirPastorRelationDirect', pastor?.id);
                                  ...
                                  <CommandGroup className='max-h-[200px] h-auto'>
                                    {pastorsQuery.data?.map((pastor) => (
                                      <CommandItem
                                        ...
                                        key={pastor.id}
                                        onSelect={() => {
                                          form.setValue('theirPastorOnlyMinistries', pastor?.id);
