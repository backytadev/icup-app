...
export const SupervisorTabsCard = ({ data, id }: SupervisorTabsCardProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      if (url.pathname === '/supervisors/inactivate')
        url.pathname = `/supervisors/inactivate/${id}/view`;
    ...
    <Tabs defaultValue='general-info' className='md:-mt-8 w-[650px] md:w-[630px]'>
      ...
      <TabsContent value='ecclesiastical-info'>
        <Card>
          ...
          <CardContent className='grid grid-cols-2 pl-[1.5rem] pr-[1rem] gap-x-6 gap-y-4 md:gap-x-10 md:gap-y-6 md:pl-[5.5rem] md:pr-[2.5rem]'>
            <div className='space-y-1'>
              <Label className='text-[14px] md:text-[15px]'>Tipo de Relación</Label>
              <CardDescription className='px-2 text-[14px] md:text-[14.5px]'>
                <li>
                  {data?.relationType
                    ? `${RelationTypeModuleNames['supervisor'][data?.relationType as RelationType]}`
