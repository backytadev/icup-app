...
export const SupervisorTabsCard = ({ data, id }: SupervisorTabsCardProps): JSX.Element => {
  useEffect(() => {
    ...
    if (id) {
      ...
      url.pathname = `/supervisors/search/${id}/view`;
  ...
  return (
    <div className='w-full md:min-w-[680px] -mt-2 md:-mt-6'>
      {/* Header */}
      <div className='relative overflow-hidden rounded-t-xl bg-gradient-to-r from-cyan-600 via-teal-600 to-cyan-700 dark:from-cyan-800 dark:via-teal-800 dark:to-cyan-900 px-4 py-4 md:px-6'>
        <div className='flex items-center justify-between gap-3'>
          <div className='flex-1 min-w-0'>
            <div className='flex items-center gap-2 mb-1.5 flex-wrap'>
              <span className='px-2 py-0.5 text-[10px] font-semibold bg-cyan-500/20 text-cyan-100 rounded font-inter uppercase tracking-wider'>
                Supervisor
              </span>
            ...
            <p className='text-cyan-100/80 text-[12px] md:text-[13px] font-inter mt-0.5 truncate'>
          ...
          <div className='bg-white dark:bg-slate-900 border border-t-0 border-slate-200/80 dark:border-slate-700/50 rounded-b-xl'>
            <div className='p-4 md:p-5 space-y-5'>
              ...
              <div className='border-t border-slate-200 dark:border-slate-700/50 pt-4'>
                <span className='text-[11px] font-semibold text-cyan-600 dark:text-cyan-400 uppercase tracking-wider font-inter'>
                  Información Eclesiástica
                </span>
                  ...
                  <div className='space-y-1'>
                    ...
                    <ul className='list-disc pl-4 text-sm text-slate-700 dark:text-slate-300 space-y-1 font-medium'>
                      {allMinistryRoles && allMinistryRoles.length > 0 ? (
                        allMinistryRoles.map((role, idx) => (
                          <li key={idx}>{MemberRoleNames[role as unknown as MemberRole]}</li>
                        ))
                      ) : (
            ...
            <div className='p-4 md:p-5 space-y-4'>
              ...
              <div className='p-3 rounded-lg bg-slate-50/80 dark:bg-slate-800/30 border border-slate-200/50 dark:border-slate-700/30'>
                <div className='flex items-start gap-2.5'>
                  <div className='p-1.5 rounded-md bg-cyan-100 dark:bg-cyan-900/30 mt-0.5'>
