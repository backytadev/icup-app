...
export const PreacherOptionsPage = (): JSX.Element => {
  ...
  return (
    <div className='animate-fadeIn'>
      <h1 className='text-center pb-2 pt-3 md:pt-2 md:pb-2 font-sans text-2xl sm:text-3xl font-bold text-preacher-color text-[2rem] sm:text-[2.4rem] md:text-[2.6rem] lg:text-5xl xl:text-5xl'>
        Modulo Predicador
      </h1>
      ...
      <div className='w-full pt-6 pb-10 px-[2rem] sm:px-[7rem] md:px-[4rem] lg:px-[3rem] xl:px-[3rem] 2xl:px-[4rem] grid gap-8 md:gap-6 2xl:gap-4 grid-cols-1 lg:grid-cols-2 2xl:grid-cols-4 lg:grid-rows-3 2xl:grid-rows-3 lg:h-[58rem] xl:min-h-screen'>
        <NavLink
          ...
          to={hasFullAccess ? '/preachers/create' : '#'}
          onClick={(e) => {
        ...
        >
          <WhiteCard disabled={!hasFullAccess} className='h-[10.5rem] md:h-[11rem]' centered>
            <FcManager className='text-[4rem] sm:text-[5rem] md:text-[6rem]' />
        ...
        <NavLink
          ...
          to={hasFullAccess ? '/preachers/update' : '#'}
          onClick={(e) => {
            if (!hasFullAccess) e.preventDefault();
          }}
          end
        ...
        <NavLink
          key='/preachers/inactivate'
          to={hasFullAccess ? '/preachers/inactivate' : '#'}
          onClick={(e) => {
            if (!hasFullAccess) e.preventDefault();
          }}
          end
        ...
        >
          <WhiteCard disabled={!hasFullAccess} className='md:h-[11rem]' centered>
            ...
            <p className='font-bold text-[13px] md:text-[14px] lg:text-[15px] xl:text-[16px]'>
              Inactivar registro de un predicador
            </p>
          </WhiteCard>
        </NavLink>
      </div>
