...
export const AlertUpdateRelationPreacher = ({
  ...
  setChangedId,
  changedId,
}: AlertUpdateRelationPreacherProps) => {
  return (
        ...
        <AlertDialogHeader className='h-auto'>
          <AlertDialogTitle className='dark:text-yellow-500 text-amber-500 font-bold text-xl text-center md:text-[28px] pb-3 flex flex-col gap-2'>
            ...
            <p className='text-[15px] md:text-[17px] font-medium text-gray-800 dark:text-gray-200 leading-5 md:leading-8'>
              Estás a punto de actualizar el Supervisor asignado al siguiente Predicador:
            </p>
          ...
          <AlertDialogDescription className='space-y-3 md:space-y-5 mt-4 text-[14px] md:text-[16px] leading-relaxed text-left text-gray-800 dark:text-gray-200'>
            <div className='bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 px-4 py-3 rounded'>
              ...
              <div className='flex justify-between flex-wrap'>
                <p>
                  ...
                  <strong>
                    {getInitialFullNames({
                      firstNames: data?.theirSupervisor?.firstNames ?? 'Sin',
                      lastNames: data?.theirSupervisor?.lastNames ?? 'Supervisor',
                    })}
                  </strong>
                </p>
              ...
              <div className='flex justify-between flex-wrap'>
                <p>
                  ...
                  <strong className='font-medium'>
                    {getInitialFullNames({
                      firstNames:
                        supervisorsQuery?.data?.find((supervisor) => supervisor.id === changedId)
                          ?.member?.firstNames ?? '',
                      lastNames:
                        supervisorsQuery?.data?.find((supervisor) => supervisor.id === changedId)
                          ?.member?.lastNames ?? '',
                ...
                <p>
                  ...
                  <strong className='font-medium'>
                    {getInitialFullNames({
                      firstNames:
                        supervisorsQuery?.data?.find((supervisor) => supervisor.id === changedId)
                          ?.theirCopastor?.member?.firstNames ?? '',
                      lastNames:
                        supervisorsQuery?.data?.find((supervisor) => supervisor.id === changedId)
                          ?.theirCopastor?.member?.lastNames ?? '',
                    })}
                  </strong>
                </p>
                <p>
            ...
            <div className='bg-red-50 dark:bg-red-900/20 border-l-4 border-red-600 px-4 py-3 rounded'>
              ...
              <p>
                Al realizar el cambio de Supervisor para este Predicador, se eliminarán sus
                relaciones anteriores y se establecerán las nuevas. Además, todo lo que que estaban
                bajo su cobertura (grupo familiar y discípulos) también serán reasignados com las
                nuevas relaciones .
              </p>
            </div>
          </AlertDialogDescription>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel
            ...
            onClick={() => {
              ...
              setChangedId(data?.theirSupervisor?.id!);
            }}
          >
            No, Cancelar
          </AlertDialogCancel>
