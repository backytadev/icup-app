...
import { usePreacherForm } from '@/modules/preacher/hooks/forms';
import { type PreacherResponse } from '@/modules/preacher/types/preacher-response.interface';
import { PreacherFormSkeleton } from '@/modules/preacher/components/PreacherFormSkeleton';
...
interface PreacherUpdateFormProps {
  id: string;
  dialogClose: () => void;
  scrollToTop: () => void;
...
export const PreacherUpdateForm = ({
  ...
  dialogClose,
  scrollToTop,
}: PreacherUpdateFormProps): JSX.Element => {
  const {
    form,
    isLoadingData,
    isInputDisabled,
    ...
    handleSubmit,
    handleRolePromotion,
  } = usePreacherForm({ mode: 'update', id, data, dialogClose, scrollToTop });
  ...
  const isPromoted = roles.includes(MemberRole.Supervisor);
  ...
  return (
    <div className='w-full max-w-[1100px] mx-auto -mt-2'>
      <div className='relative overflow-hidden rounded-t-xl bg-gradient-to-r from-violet-500 via-purple-500 to-violet-600 dark:from-violet-700 dark:via-purple-700 dark:to-violet-800 px-6 py-5'>
          ...
          <Form {...form}>
            ...
            >
              ...
              <div className='sm:col-start-3 sm:col-end-4 flex flex-col gap-4'>
                ...
                {/* Alert: supervisor change confirmation */}
                ...
                <div className='flex flex-col gap-3 rounded-lg border border-amber-200 dark:border-amber-800/50 bg-amber-50/60 dark:bg-amber-900/10 p-3'>
                  ...
                  {/* Post-promotion: choose path (direct to pastor or via copastor) */}
                  {isPromoteButtonDisabled && isInputDisabled && isPromoted && (
                    <div className='flex flex-col gap-2'>
                      <div className='flex items-center gap-2 p-2 rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700'>
                        <Checkbox
                          id='isDirectRelationToPastor'
                          checked={isDirectRelationToPastor}
                          onCheckedChange={(checked) =>
                            form.setValue('isDirectRelationToPastor', !!checked)
                          }
                          disabled={isRelationSelectDisabled}
                          className='data-[state=checked]:bg-amber-500 data-[state=checked]:border-amber-500'
                        />
                        <Label
                          htmlFor='isDirectRelationToPastor'
                          className='text-[12px] font-medium cursor-pointer text-slate-700 dark:text-slate-300'
                        >
                  ...
                  {isPromoteButtonDisabled &&
                    ...
                    !theirPastorRelationDirect && (
                      <p className='text-[11.5px] font-bold text-center text-red-500'>
                        ⚠️ Debes asignar un Co-Pastor o Pastor directo al nuevo Supervisor antes de
                        guardar.
                      </p>
                    )}
                  {isPromoteButtonDisabled &&
                    isPromoted &&
                    (theirCopastor || theirPastorRelationDirect) && (
