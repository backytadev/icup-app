...
import { useZoneInactivationMutation } from '@/modules/zone/hooks/mutations';
import { zoneInactivationFormSchema } from '@/modules/zone/schemas';
...
export const ZoneInactivateCard = ({ idRow }: ZoneInactivateCardProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (idRow && isCardOpen) {
      ...
      url.pathname = `/zones/search/${idRow}/remove`;
          ...
          <div className='relative overflow-hidden bg-gradient-to-r from-red-500 via-red-600 to-rose-600 dark:from-red-700 dark:via-red-800 dark:to-rose-800 p-5 rounded-t-lg'>
            ...
            <div className='relative z-10'>
              ...
              <DialogDescription className='text-red-100/80 text-sm text-center mt-1 font-inter'>
                Esta acción desactivará el registro de la zona
              </DialogDescription>
            </div>
          </div>
            ...
            <div className='p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30'>
              ...
              <ul className='space-y-2 text-[13px] text-amber-600 dark:text-amber-300/90 font-inter'>
                ...
                <li className='flex items-start gap-2'>
                  <span className='text-red-500'>✗</span>
                  <span>
                    Se eliminará de relaciones con: Pastor, Co-Pastor, Supervisor, Predicador, G. Familiar, Discípulo
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <span className='text-emerald-500'>✓</span>
                  <span>
                    Podrás reactivarlo desde <span className='font-semibold'>Actualizar Zona</span>
                  </span>
                </li>
              </ul>
            </div>
                      ...
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent className='mr-[0rem] w-[85%] sm:ml-[0rem] sm:w-[95%] md:mx-auto md:w-full'>
                            {Object.entries(
                              zoneInactivationCategory === ZoneInactivationCategory.AdministrativeChanges
                                ? AdministrativeChangesReasonsNames
                                : zoneInactivationCategory === ZoneInactivationCategory.ExternalFactors
                                  ? ExternalFactorsReasonsNames
                                  : zoneInactivationCategory === ZoneInactivationCategory.FamilyGroupsRelatedReasons
                                    ? GroupFamilyRelatedReasonsReasonsNames
                                    : zoneInactivationCategory === ZoneInactivationCategory.LackOfActivityOrCommitment
                                      ? LackOfActivityOrCommitmentReasonsNames
                                      : zoneInactivationCategory === ZoneInactivationCategory.LeadershipIssues
                                        ? LeadershipIssuesReasonsNames
                                        : zoneInactivationCategory === ZoneInactivationCategory.UnavoidableCircumstances
                                          ? UnavoidableCircumstancesReasonsNames
                                          : []
                            ).map(([key, value]) => (
                              <SelectItem className='text-[13px] md:text-[14px] font-inter' key={key} value={key}>
