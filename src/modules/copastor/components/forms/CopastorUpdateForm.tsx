...
import { CopastorFormSkeleton } from '@/modules/copastor/components/CopastorFormSkeleton';
...
interface CopastorUpdateFormProps {
  id: string;
  dialogClose: () => void;
  scrollToTop: () => void;
...
export const CopastorUpdateForm = ({
  ...
  dialogClose,
  scrollToTop,
}: CopastorUpdateFormProps): JSX.Element => {
  const {
    form,
    isLoadingData,
    isInputDisabled,
    ...
    handleSubmit,
    handleRolePromotion,
  } = useCopastorForm({ mode: 'update', id, data, dialogClose, scrollToTop });
  ...
  const isPromoted = roles.includes(MemberRole.Pastor);
  const hasPresbyterRole = roles.includes(MemberRole.Presbyter);
          ...
          <Form {...form}>
            ...
            >
              ...
              <div className='sm:col-start-3 sm:col-end-4 flex flex-col gap-4'>
                ...
                {/* Alert: pastor change confirmation (controlled via isAlertDialogOpen) */}
                ...
                <div className='flex flex-col gap-3 rounded-lg border border-amber-200 dark:border-amber-800/50 bg-amber-50/60 dark:bg-amber-900/10 p-3'>
                  ...
                  {/* Church selector after promotion */}
                  ...
                  {/* Feedback messages */}
                  {isPromoteButtonDisabled && !theirChurch && isPromoted && (
                    <p className='text-[11.5px] font-bold text-center text-red-500'>
                      ⚠️ Debes asignar una iglesia al nuevo Pastor antes de guardar.
                    </p>
                  )}
                  {isPromoteButtonDisabled && theirChurch && isPromoted && (
                    <p className='text-[11.5px] font-bold text-center text-green-500'>
                      ✅ Iglesia asignada. Guarda los cambios para completar la promoción.
                    </p>
                  )}
